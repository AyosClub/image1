...
function library:createConfig()
    ...
    writefile("silenthook11/"..name..".cfg",game:GetService("HttpService"):JSONEncode(jig))
...
function library:loadConfig()
    local name = library.flags["selected_config"]
    if not isfile("silenthook11/"..name..".cfg") then
        library:notify("Config file not found!")
        return
    end
    local config = game:GetService("HttpService"):JSONDecode(readfile("silenthook11/"..name..".cfg"))
...
function library:refreshConfigs()
    local tbl = {}
    for i,v in next, listfiles("silenthook11") do
        table.insert(tbl,v)
    end
...
function library:deleteConfig()
    if isfile("silenthook11/"..library.flags["selected_config"]..".cfg") then
        delfile("silenthook11/"..library.flags["selected_config"]..".cfg")
        library:refreshConfigs()
    end
end
